  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pendingScenes.length; i += BATCH_SIZE) {
         ...
         if (i + BATCH_SIZE < pendingScenes.length) await new Promise(r => setTimeout(r, 1000));
      }
    } catch (err) { 
      console.error(err); 
    } finally { 
      ...
      <div className="flex flex-col xl:flex-row gap-10">
        <div className="flex-1 flex flex-col gap-10">
          <div className="bg-slate-900 border border-slate-800 p-10 rounded-[3.5rem] shadow-2xl space-y-10 relative overflow-hidden ring-1 ring-slate-800/50">
            <div className="flex items-center justify-between relative z-10"><div className="flex items-center gap-3"><div className="w-14 h-14 bg-purple-600/20 rounded-2xl flex items-center justify-center text-purple-400 shadow-inner border border-purple-500/20"><Book size={28}/></div><div><h2 className="text-3xl font-black text-white uppercase tracking-tight">Narrative Direction</h2><p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Documentary Grade Scripting</p></div></div>{state.script && (<button onClick={handleSummarize} disabled={isSummarizing} className="flex items-center gap-2 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-700 shadow-xl transition-all active:scale-95 disabled:opacity-50">{isSummarizing ? <Loader2 size={14} className="animate-spin" /> : <ListChecks size={14} />}Summarize Script</button>)}</div>
            <div className="space-y-8 relative z-10"><div className="relative group"><input type="text" placeholder="Enter Documentary Focus..." className="w-full bg-slate-950 border border-slate-800 rounded-3xl p-7 pr-48 text-white text-xl font-kanit outline-none shadow-inner focus:ring-4 focus:ring-purple-600/10 transition-all shadow-inner placeholder:text-slate-700" value={state.topic} onChange={(e) => setState(prev => ({ ...prev, topic: e.target.value }))} /><button onClick={handleGenerateScript} disabled={isGenerating} className="absolute right-3.5 top-3.5 bottom-3.5 px-10 bg-purple-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-purple-500 shadow-2xl transition-all disabled:opacity-50 active:scale-95">{isGenerating ? <Loader2 className="animate-spin" /> : 'Draft Script'}</button></div><div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="bg-slate-950 p-5 rounded-[1.5rem] border border-slate-800 hover:border-slate-700 transition-colors">
                ...
                <select value={selectedTextModel} onChange={(e) => setSelectedTextModel(e.target.value)} className="w-full bg-transparent text-white font-bold outline-none text-[13px] cursor-pointer">
                  <option value="gemini-3-flash-preview" className="bg-slate-900">Gemini 3 Flash</option>
                  <option value="gemini-3-pro-preview" className="bg-slate-900">Gemini 3 Pro</option>
                </select>
              </div>
              ...
              <div className="bg-slate-950 p-5 rounded-[1.5rem] border border-slate-800 hover:border-slate-700 transition-colors">
                ...
                <button 
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl py-2 px-3 text-white font-bold text-[13px] flex items-center justify-between hover:bg-slate-800 transition-all"
                >
                        ...
                        {!currentVideoBlob && (
                          <div className="flex items-center gap-4 p-6 bg-slate-900/50 border border-slate-800 rounded-3xl">
                              ...
                              <p className="text-[10px] text-slate-500 font-medium leading-relaxed font-kanit">
                                ...
                                สำหรับงานวิดีโอยาว แนะนำให้ใช้ "Autonomous Queue" เพื่อเรนเดอร์ในเบื้องหลัง ระบบจะทำการเรนเดอร์และอัปโหลดให้คุณโดยที่คุณไม่ต้องรอเปิดหน้าต่างนี้ค้างไว้
                              </p>
                          </div>
                        )}
                      </div>
